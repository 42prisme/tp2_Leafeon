...
module.exports = (list, item, user) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            await user.dao.db.query("CREATE TABLE users(id SERIAL, name TEXT UNIQUE, email TEXT, password TEXT)")
            const Hash = new hash()
            //console.log("password hashing",Hash.hashPassword("Jackson"))
            await user.dao.insert("hugo","hugo@mail.com", Hash.hashPassword("Jackson"))
            await user.dao.insert("admin","hugo@mail.com", Hash.hashPassword("admin"))
            await user.dao.insert("Papa","hugo@mail.com", Hash.hashPassword("Papacleme1l!eur"))
